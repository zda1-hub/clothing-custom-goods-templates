...
function HatModel({ felt, finish, bandColor, markLabel }: {
  felt: string; finish: string; bandColor: string; markLabel: string;
}) {
  ...
  return <Center><group>
    ...
    <group rotation={[-Math.PI / 2, 0, 0]}>
      ...
      <mesh position={[0, -5.6, 3.72]} rotation={[Math.PI / 2, 0, 0]} renderOrder={8}>
...
export default function Showcase() {
  ...
  const [engraving, setEngraving] = useState("Monogram"); const [bandColor, setBandColor] = useState("Espresso"); const [requested, setRequested] = useState(false);
  ...
  return <main>
    ...
    <section className="hat-lab" id="atelier"><div className="lab-intro"><p className="section-index">02 — Shape yours</p><h2>Meet your<br/><em>future hat.</em></h2><p>Choose the felt, finish, band, and crown mark. Every option updates instantly in a clean front-facing preview.</p></div>
      <div className="hat-stage local-3d-stage"><div className="orbit-label"><span>Front view · live customization</span><span>Cava 3D atelier</span></div><HatViewer felt={felt.value} finish={finish} bandColor={bandColors[bandColor]} markLabel={markLabel}/><div className="shaper viewer-help"><span>Real-time material preview</span><span>Engraving fixed to crown</span></div></div>
